...
class MiniCourt():
    def __init__(self, frame):
        ...
        self.padding_from_overriding_color = 2  # for not coloring again the same pixel. i.e when i draw the surface and after it the table its not looking good without it
    ...
    def draw_mini_court_table(self, frame):
        ...
        # draw the left leg of the the table
    ...
    def draw_ball(self,game):
        table_lentgh=game.table.get_bottom_right()[0]-game.table.get_top_left()[0]
        table_height=game.table.get_top_left()[1]-game.table.get_bottom_right()[1]
        table_size=math.sqrt(table_lentgh**2+table_height**2)
        tmp_positions=game.ball.get_hit_positions()

        # tmp_positions=[(x,Color.RED if x>self.net_bottom_x else Color.TEAL) for x in game.ball.get_hit_positions()]


        for position in tmp_positions:
            distance_ball = calculate_linear_distance(position,game.table.get_top_left())
            x_mini_court=int(self.table_top_x+(distance_ball/table_size)*(self.table_bottom_x-self.table_top_x))

            # x_mini =int(self.table_top_x+(position[0]/(game.table.list[2]-game.table.list[0]))*(self.table_bottom_x-self.table_top_x))
            cv2.circle(VideoHandler.frame, (x_mini_court, self.table_top_y-5), 5, Color.RED, cv2.FILLED)



    # def set_real_net_location(self,game):
    #     self.left_x_net = game.table.netlist[0]
    #     self.right_x_net = game.table.netlist[2]
