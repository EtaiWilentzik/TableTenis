...
class VideoHandler:
    ...
    def paint_ball_movement(self,game):
        ...
        for i, pos in enumerate(tmp_positions):
            ...
            if i != 0:
                cv2.line(VideoHandler.frame, (pos.x, pos.y), (tmp_positions[i - 1].x, tmp_positions[i - 1].y), Color.BLACK,
                         2)
    ...
    def draw_result(self):
        ...
        # Draw the rectangle
        cv2.rectangle(VideoHandler.frame, (int(top_left_x), int(top_left_y)), (int(bottom_right_x), int(bottom_right_y)),
